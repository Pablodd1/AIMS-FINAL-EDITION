...
import { Activity, Bluetooth, Wifi, Keyboard, Zap, ArrowRight, Check } from 'lucide-react';
import { 
...
import { AlertCircle, Loader2, Smartphone, Usb, FileText } from 'lucide-react';
...
export default function BluetoothConnect({
  ...
  const [connectionTab, setConnectionTab] = useState('bluetooth');
  const [showConnectionWizard, setShowConnectionWizard] = useState(false);
  const [usbConnecting, setUsbConnecting] = useState(false);
  const serialSupported = isSerialSupported();

  const supportedDevices = getSupportedDevices(deviceType);

  const handleUSBConnect = async () => {
    setUsbConnecting(true);
    try {
      const result = await connectSerialDevice(deviceType);
      if (result) {
        setDevice(result.device as any);
        toast({
        ...
        toast({
          title: 'Connection Failed',
          description: 'No USB device selected or connection cancelled',
          variant: 'destructive',
        });
      }
    } catch (error) {
      toast({
        title: 'USB Error',
        description: 'Failed to connect to USB device',
        variant: 'destructive',
      });
    } finally {
  ...
  return (
    <div className="mb-4">
      ...
      >
        ...
        Connect Device (Bluetooth, USB, or Manual)
      </Button>

      <Dialog open={showConnectionWizard} onOpenChange={setShowConnectionWizard}>
        <DialogContent className="max-w-md">
          <DialogHeader>
            <DialogTitle>Connect RPM Device</DialogTitle>
            <DialogDescription>
              Choose your connection method. For best experience, use Bluetooth.
            </DialogDescription>
          </DialogHeader>
          ...
          <Tabs value={connectionTab} onValueChange={setConnectionTab} className="w-full">
            ...
            <TabsContent value="bluetooth" className="space-y-4">
              <div className="space-y-2">
                <p className="text-sm font-medium">Supported Bluetooth Devices:</p>
                <div className="max-h-32 overflow-y-auto space-y-1">
                  {supportedDevices.filter(d => d.protocol === 'bluetooth').map((dev, i) => (
                    <div key={i} className="text-xs bg-gray-50 p-2 rounded flex items-center gap-2">
              ...
              <Button
                ...
                className="w-full"
              >
                {isConnecting ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Bluetooth className="h-4 w-4 mr-2" />}
                {isConnecting ? 'Scanning...' : 'Scan for Bluetooth Devices'}
              </Button>
              {!isBluetoothAvailable() && (
                <Alert variant="destructive">
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>Browser doesn't support Bluetooth. Use Manual entry.</AlertDescription>
                </Alert>
              )}
            </TabsContent>
            ...
            <TabsContent value="usb" className="space-y-4">
              <div className="space-y-2">
                <p className="text-sm font-medium">USB Serial Devices:</p>
                <div className="text-xs bg-gray-50 p-2 rounded">
                  <Usb className="h-4 w-4 inline mr-2" />
                  Connect via USB cable (some devices support this)
                </div>
              </div>
              <Button
                onClick={handleUSBConnect}
                disabled={usbConnecting || !serialSupported}
                className="w-full"
              >
                {usbConnecting ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Usb className="h-4 w-4 mr-2" />}
                {usbConnecting ? 'Connecting...' : 'Connect USB Device'}
              </Button>
              {!serialSupported && (
                <Alert variant="destructive">
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>WebUSB not supported. Use Manual entry.</AlertDescription>
                </Alert>
              )}
            </TabsContent>
            ...
            <TabsContent value="manual" className="space-y-4">
              <Alert>
                ...
                <AlertDescription>
                  Enter readings manually from any device display
                </AlertDescription>
              </Alert>
              <Button
                onClick={() => {
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setShowConnectionWizard(false)}>
              Cancel
            </Button>
          </DialogFooter>
        </DialogContent>
      ...
      {device && (
        <div className="flex space-x-2 mt-4">
          ...
          <Button
            variant="destructive"
            onClick={async () => {
              await disconnectDevice(device as any);
              setDevice(null);
