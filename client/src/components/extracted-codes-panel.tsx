...
import { COMMON_ICD10_CODES, COMMON_CPT_CODES, MedicalCode } from "@/lib/medical-codes";
...
export function ExtractedCodesPanel({
  ...
  const [isAddingDx, setIsAddingDx] = useState(false);
  const [isAddingCpt, setIsAddingCpt] = useState(false);
  const [searchTerm, setSearchTerm] = useState("");
  const { toast } = useToast();

  const handleAddCode = (type: 'icd10' | 'cpt', code: MedicalCode) => {
    if (onCodeEdit) {
      const newCode = {
        code: code.code,
        description: code.description,
        rationale: "Manually added by provider",
        confidence: 'high' as const
      };

      const index = type === 'icd10' ? icd10Codes.length : cptCodes.length;
      onCodeEdit(type, index, newCode);

      toast({
        title: "Code Added",
        description: `${code.code} added to report`,
      });
                ...
                <TableBody>
                  ...
                  <TableRow className="bg-muted/30">
                    <TableCell colSpan={4} className="p-2">
                      <Popover open={isAddingDx} onOpenChange={setIsAddingDx}>
                        <PopoverTrigger asChild>
                          <Button variant="ghost" className="w-full justify-start gap-2 h-9 text-xs border border-dashed border-primary/30">
                            <Plus className="h-4 w-4" />
                            Add Diagnosis (Search Database)
                          </Button>
                        </PopoverTrigger>
                        <PopoverContent className="w-[400px] p-0" align="start">
                          <Command>
                            <CommandInput placeholder="Search ICD-10 database..." />
                            <CommandList>
                              ...
                              <CommandGroup heading="Common ICD-10 Codes">
                                {COMMON_ICD10_CODES.map((code) => (
                                  <CommandItem
                                    key={code.code}
                                    onSelect={() => handleAddCode('icd10', code)}
                ...
                <TableBody>
                  ...
                  <TableRow className="bg-muted/30">
                    <TableCell colSpan={5} className="p-2">
                      <Popover open={isAddingCpt} onOpenChange={setIsAddingCpt}>
                        <PopoverTrigger asChild>
                          <Button variant="ghost" className="w-full justify-start gap-2 h-9 text-xs border border-dashed border-primary/30">
                            <Plus className="h-4 w-4" />
                            Add Procedure (Search Database)
                          </Button>
                        </PopoverTrigger>
                        <PopoverContent className="w-[400px] p-0" align="start">
                          <Command>
                            <CommandInput placeholder="Search CPT database..." />
                            <CommandList>
                              <CommandEmpty>No results found.</CommandEmpty>
                              <CommandGroup heading="Common CPT Codes">
                                {COMMON_CPT_CODES.map((code) => (
                                  <CommandItem
                                    key={code.code}
                                    onSelect={() => handleAddCode('cpt', code)}
                                    className="cursor-pointer"
                                  >
                                    <span className="font-mono font-bold mr-2">{code.code}</span>
                                    <span className="text-muted-foreground truncate">{code.description}</span>
