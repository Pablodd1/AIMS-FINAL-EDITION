...
// Patient-only navigation items
const patientNavigation = [
  { name: "My Portal", href: "/patient-portal", icon: UserCircle, patientOnly: true },
];
...
export default function Sidebar() {
  ...
  const SidebarLink = ({ item, isCollapsed }: SidebarLinkProps) => {
    ...
    const navKey = `nav.${item.href.split('/').filter(Boolean).join('')}`;
    const displayName = t(navKey) || item.name;
      ...
      <div className={cn("px-3 py-4 border-t", { "text-center": isCollapsed })}>
        ...
        <div className="mb-4">
          <Button
            variant="outline"
            size="sm"
            className={cn("w-full gap-2", isCollapsed ? "px-0 justify-center" : "justify-start")}
            onClick={() => setLanguage(language === 'en' ? 'es' : 'en')}
          >
            <Globe className="h-4 w-4" />
            {!isCollapsed && (language === 'en' ? 'English' : 'Español (VZ)')}
          </Button>
        </div>
