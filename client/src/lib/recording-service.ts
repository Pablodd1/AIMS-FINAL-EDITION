...
class BrowserRecordingService implements RecordingServiceInterface {
  ...
  async stopRecording(language: string = 'en-US'): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.mediaRecorder.addEventListener('stop', async () => {
        try {
          ...
          this.transcriptText = await this.transcribeAudio(audioBlob, language);
  ...
  async processAudioFile(file: File, language: string = 'en-US'): Promise<string> {
    try {
      ...
      this.transcriptText = await this.transcribeAudio(file, language);
      return this.transcriptText;
    } catch (error) {
  ...
  private async transcribeAudio(audioBlob: Blob | File, language: string = 'en-US'): Promise<string> {
    try {
...
): Promise<{ soap: string; structuredData?: any }> {
  try {
    ...
    // Return the soap notes and structured data
    return {
      soap: data.soap || "No SOAP notes were generated. Please try again.",
    ...
    return {
      soap: "There was an error generating SOAP notes. Please try again."
    };
  }
}
