...
export default function Analytics() {
  ...
  // Fetch medical insights from Python service
  const { data: medicalInsights, isLoading: isLoadingInsights } = useQuery({
    queryKey: ['/api/python-analytics/generate-insights', selectedPatientId],
    queryFn: async () => {
      const sampleLabs = [
        { name: 'Glucose', value: 110, unit: 'mg/dL', reference_range_min: 70, reference_range_max: 99, category: 'Metabolic' },
        { name: 'Cholesterol', value: 220, unit: 'mg/dL', reference_range_min: 125, reference_range_max: 200, category: 'Lipids' },
        { name: 'ALT', value: 45, unit: 'U/L', reference_range_min: 7, reference_range_max: 55, category: 'Liver' },
        { name: 'TSH', value: 3.5, unit: 'mIU/L', reference_range_min: 0.4, reference_range_max: 4.0, category: 'Thyroid' }
      ];

      try {
        const response = await fetch('/api/python-analytics/generate-insights', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            patient_id: selectedPatientId || 0,
            lab_values: sampleLabs
          })
        });
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <CardContent>
            ...
            <div className="mt-1 flex items-center">
              <Badge className="bg-blue-800">
                {healthScore > 75 ? t('analytics.excellent') : healthScore > 60 ? t('analytics.good') : t('analytics.attention')}
              </Badge>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t('analytics.patientGrowth')}</CardTitle>
            <TrendingUp className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">+2,350</div>
            <p className="text-xs text-muted-foreground">+15.2% {t('dashboard.stats.trend.month')}</p>
          </CardContent>
        </Card>

        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">{t('analytics.totalReadings')}</CardTitle>
            <Activity className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{healthData.bloodPressure.length + healthData.glucose.length}</div>
            <p className="text-xs text-muted-foreground">{t('dashboard.stats.trend.week')}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">+4.3% {t('dashboard.stats.trend.month')}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle>{t('analytics.bpTrends')}</CardTitle>
              <div className="flex space-x-2">
                <div className="flex items-center space-x-1">
                  <div className="h-3 w-3 rounded-full bg-blue-500" />
                  <span className="text-sm text-muted-foreground">{t('analytics.systolic')}</span>
                </div>
                <div className="flex items-center space-x-1">
                  <div className="h-3 w-3 rounded-full bg-red-500" />
                  <span className="text-sm text-muted-foreground">{t('analytics.diastolic')}</span>
                </div>
              </div>
            </div>
            <CardDescription>
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle>{t('analytics.glucoseMonitoring')}</CardTitle>
              <div className="flex space-x-2">
                <div className="flex items-center space-x-1">
                  <div className="h-3 w-3 rounded-full bg-green-500" />
                  <span className="text-sm text-muted-foreground">{t('analytics.fasting')}</span>
                </div>
                <div className="flex items-center space-x-1">
                  <div className="h-3 w-3 rounded-full bg-orange-500" />
                  <span className="text-sm text-muted-foreground">{t('analytics.postMeal')}</span>
                </div>
              </div>
            </div>
            <CardDescription>
              {t('analytics.glucoseDesc')}
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="h-[350px]">
      ...
      <Tabs defaultValue="health" className="space-y-4">
        <TabsList className="grid grid-cols-5 md:w-[750px]">
          <TabsTrigger value="health">{t('analytics.tabHealth')}</TabsTrigger>
          <TabsTrigger value="medical-ai">{t('analytics.tabMedicalAI')}</TabsTrigger>
          <TabsTrigger value="patients">{t('analytics.tabPatients')}</TabsTrigger>
          <TabsTrigger value="practice">{t('analytics.tabPractice')}</TabsTrigger>
          <TabsTrigger value="devices">{t('analytics.tabDevices')}</TabsTrigger>
        </TabsList>

        <TabsContent value="medical-ai">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-primary flex items-center gap-2">
                      ...
                      {t('analytics.pythonInsights')}
                    </CardTitle>
                    <CardDescription>{t('analytics.advAnalysis')}</CardDescription>
                  </div>
                  {medicalInsights?.success && (
                    <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
                      {t('analytics.serviceOnline')}
                    </Badge>
                  )}
                </div>
              </CardHeader>
                ...
                {isLoadingInsights ? (
                  ...
                  <div className="grid gap-6 md:grid-cols-2">
                    <div className="space-y-4">
                      <h3 className="font-semibold text-lg">{t('analytics.execSummary')}</h3>
                      <div className="grid grid-cols-2 gap-4">
                        <div className="p-4 rounded-lg bg-slate-50 border">
                          <p className="text-sm text-muted-foreground">{t('analytics.healthScore')}</p>
                          <p className="text-2xl font-bold">{medicalInsights.data.executive_summary.overall_health_score}/100</p>
                        </div>
                        <div className="p-4 rounded-lg bg-slate-50 border">
                          <p className="text-sm text-muted-foreground">{t('analytics.abnormalMarkers')}</p>
                          <p className="text-2xl font-bold text-red-600">{medicalInsights.data.executive_summary.abnormal_markers_count}</p>
                        </div>
                        <div className="p-4 rounded-lg bg-slate-50 border">
                          <p className="text-sm text-muted-foreground">{t('analytics.highRiskAreas')}</p>
                          <p className="text-2xl font-bold text-orange-600">{medicalInsights.data.executive_summary.high_risk_areas}</p>
                        </div>
                        <div className="p-4 rounded-lg bg-slate-50 border">
                          <p className="text-sm text-muted-foreground">{t('analytics.statOutliers')}</p>
                          <p className="text-2xl font-bold">{medicalInsights.data.executive_summary.outliers_detected}</p>
                        </div>
                      </div>

                      <div className="space-y-2 pt-4">
                        <h4 className="font-medium">{t('analytics.actionableInsights')}</h4>
                        {medicalInsights.data.actionable_insights.map((insight: any, idx: number) => (
                          <div key={idx} className={`p-3 rounded-md border-l-4 ${insight.priority === 'high' ? 'bg-red-50 border-red-500' : 'bg-orange-50 border-orange-500'}`}>
                            <p className="font-semibold text-sm">{insight.insight}</p>
                            <p className="text-xs text-muted-foreground mt-1">Action: {insight.action}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    <div className="space-y-4">
                      <h3 className="font-semibold text-lg">{t('analytics.riskAssessments')}</h3>
                      <div className="space-y-3">
                        {Object.entries(medicalInsights.data.detailed_analysis.risk_assessment.risk_assessments).map(([key, value]: [string, any]) => (
                          <div key={key} className="space-y-1">
                            <div className="flex justify-between text-sm">
                              <span className="capitalize">{key} {t('analytics.riskAnalysis')}</span>
                              <span className={`font-medium ${value.risk_level === 'high' ? 'text-red-600' : value.risk_level === 'moderate' ? 'text-orange-600' : 'text-green-600'}`}>
                                {value.risk_level.toUpperCase()} ({Math.round(value.risk_percentage)}%)
                              </span>
                            </div>
                            <div className="w-full bg-slate-100 rounded-full h-2">
                              <div
                                className={`h-2 rounded-full ${value.risk_level === 'high' ? 'bg-red-500' : value.risk_level === 'moderate' ? 'bg-orange-500' : 'bg-green-500'}`}
                                style={{ width: `${value.risk_percentage}%` }}
                              />
                            </div>
                          </div>
                        ))}
                      ...
                      <div className="space-y-2 pt-4">
                        <h4 className="font-medium">{t('analytics.recommendations')}</h4>
                        <ul className="list-disc list-inside text-sm space-y-1 text-muted-foreground">
                          {medicalInsights.data.follow_up_recommendations.map((rec: string, idx: number) => (
                            <li key={idx}>{rec}</li>
                          ))}
                        </ul>
                      </div>
                  ...
                  <div className="text-center py-12 text-muted-foreground">
                    ...
                    <p>{t('analytics.serviceOffline')}</p>
                    <p className="text-sm">{t('analytics.serviceOfflineDesc')}</p>
                  </div>
                )}
              </CardContent>
            </Card>
        ...
        <TabsContent value="health">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <Card>
              <CardHeader>
                <CardTitle>{t('analytics.riskAnalysis')}</CardTitle>
        ...
        <TabsContent value="practice">
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader>
                <CardTitle>{t('analytics.appointments')}</CardTitle>
        ...
        <TabsContent value="devices">
          <div className="grid gap-4 md:grid-cols-2">
            <Card>
              <CardHeader>
                <CardTitle>{t('analytics.deviceDist')}</CardTitle>
            ...
            <Card>
              <CardHeader>
                <CardTitle>{t('analytics.deviceFreq')}</CardTitle>
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center pt-2">
        <div className="flex items-center gap-3">
          <Button variant="outline" size="sm" className="gap-2">
            <Filter className="h-4 w-4" /> {t('analytics.filters')}
          </Button>
          ...
          <div className="flex items-center space-x-2">
            ...
            />
            <Label htmlFor="show-anomalies">{t('analytics.showAnomalies')}</Label>
          </div>
        </div>
