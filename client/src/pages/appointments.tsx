    ...
    <div className="space-y-4 md:space-y-6 p-4 md:p-0">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <Dialog open={isPatientListDialogOpen} onOpenChange={setIsPatientListDialogOpen}>
            <DialogTrigger asChild>
              <Button variant="outline" data-testid="button-send-patient-list" className="w-full sm:w-auto">
                ...
                <span className="hidden sm:inline">{t('appointments.sendList')}</span>
                <span className="sm:hidden">{t('appointments.sendList')}</span>
              </Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{t('appointments.sendList')}</DialogTitle>
              </DialogHeader>
          ...
          <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{t('appointments.scheduleTitle')}</DialogTitle>
              </DialogHeader>
              <Form {...form}>
                <form onSubmit={form.handleSubmit((data) => createAppointmentMutation.mutate(data))} className="space-y-4">
                      ...
                      <FormItem className="flex flex-col">
                        ...
                        <Popover open={patientSearchOpen} onOpenChange={setPatientSearchOpen}>
                          ...
                          <PopoverContent className="w-[90vw] sm:w-[400px] p-0">
                            <Command>
                              <CommandInput placeholder={t('appointments.searchPatients')} />
                              <CommandList>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem className="flex flex-col">
                        <FormLabel>{t('appointments.dateTime')}</FormLabel>
                        <Popover>
                          <PopoverTrigger asChild>
                            <FormControl>
                              <Button
                  ...
                  <Button type="submit" className="w-full" disabled={createAppointmentMutation.isPending}>
                    {createAppointmentMutation.isPending ? t('common.pending') : t('appointments.button.schedule')}
                  </Button>
                </form>
              </Form>
            </DialogContent>
      ...
      <Tabs defaultValue="list" className="w-full" onValueChange={(value) => setViewMode(value as "list" | "calendar")}>
        <TabsList className="grid w-full grid-cols-2">
          <TabsTrigger value="list">{t('appointments.view.list')}</TabsTrigger>
          <TabsTrigger value="calendar">{t('appointments.view.calendar')}</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="list" className="mt-4">
          ...
          <Card className="mb-6">
            <CardHeader className="pb-3">
              <CardTitle className="text-lg">{t('appointments.search.title')}</CardTitle>
            </CardHeader>
            <CardContent>
                ...
                >
                  <CardContent className="p-3">
                    <div className="flex items-center gap-1 mb-1">
                      ...
                      <span className="text-white text-xs font-medium">{t('appointments.status.scheduled')}</span>
                    </div>
                ...
                >
                  <CardContent className="p-3">
                    <div className="flex items-center gap-1 mb-1">
                      ...
                      <span className="text-white text-xs font-medium">{t('appointments.status.cancelled')}</span>
                    </div>
                    <div className="text-xl font-bold text-white">
                ...
                >
                  <CardContent className="p-3">
                    <div className="flex items-center gap-1 mb-1">
                      ...
                      <span className="text-white text-xs font-medium">{t('appointments.status.completed')}</span>
                    </div>
                    <div className="text-xl font-bold text-white">
                ...
                >
                  <CardContent className="p-3">
                    <div className="flex items-center gap-1 mb-1">
                      ...
                      <span className="text-white text-xs font-medium">{t('appointments.status.pending')}</span>
                    </div>
                    <div className="text-xl font-bold text-white">
                ...
                >
                  <CardContent className="p-3">
                    <div className="flex items-center gap-1 mb-1">
                      ...
                      <span className="text-white text-xs font-medium">{t('appointments.status.all')}</span>
                    </div>
                    <div className="text-xl font-bold text-white">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                <div className="space-y-1">
                  <label className="text-sm font-medium">{t('patients.table.name')}</label>
                  <Input
                    placeholder={t('appointments.search.name')}
                    value={searchName}
                ...
                <div className="space-y-1">
                  <label className="text-sm font-medium">{t('patients.table.email')}</label>
                  <Input
                    placeholder={t('appointments.search.email')}
                    value={searchEmail}
                ...
                <div className="space-y-1">
                  <label className="text-sm font-medium">{t('patients.table.status')}</label>
                  <Select value={searchStatus} onValueChange={setSearchStatus}>
                    <SelectTrigger data-testid="select-search-status">
                      <SelectValue placeholder={t('appointments.search.status')} />
                    </SelectTrigger>
                    <SelectContent>
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex gap-2 mt-2">
                      <Badge variant={
                        ...
                        {t('appointments.confirmation')}: {(appointment as any).patientConfirmationStatus === "pending_confirmation" ? t('appointments.status.pending') :
                          (appointment as any).patientConfirmationStatus === "confirmed" ? t('appointments.status.confirmed') :
                            (appointment as any).patientConfirmationStatus === "declined" ? t('appointments.status.declined') :
                              (appointment as any).patientConfirmationStatus || t('appointments.status.pending')}
                      </Badge>
                    </div>
                  </div>
                    ...
                    <Popover>
                      <PopoverTrigger asChild>
                        <Button variant="outline" size="sm" className="w-full sm:w-auto">
                          ...
                          <span className="hidden sm:inline">{t('appointments.updateStatus')}</span>
                          <span className="sm:hidden">{t('appointments.updateStatus')}</span>
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-[90vw] sm:w-80" align="end">
                        <div className="space-y-4">
                          <h4 className="font-medium">{t('appointments.updateStatus')}</h4>
                          ...
                          <div>
                            <label className="text-sm font-medium">{t('appointments.customStatus')}:</label>
                            <div className="flex mt-1 gap-2">
                              <Input
                                placeholder={t('appointments.customStatus')}
                                value={customStatus}
        ...
        <TabsContent value="calendar" className="mt-4">
          ...
          <div className="mb-6">
            <h2 className="text-xl font-semibold mb-4">{t('appointments.reports')}</h2>
              ...
              >
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-white text-sm md:text-base">{t('appointments.status.scheduled')}</CardTitle>
                  </div>
                </CardHeader>
                <CardContent>
              ...
              >
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-white text-sm md:text-base">{t('appointments.status.cancelled')}</CardTitle>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="text-3xl md:text-4xl font-bold text-white mb-3">
              ...
              >
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-white text-sm md:text-base">{t('appointments.status.completed')}</CardTitle>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="text-3xl md:text-4xl font-bold text-white mb-3">
              ...
              >
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-white text-sm md:text-base">{t('appointments.status.pending')}</CardTitle>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="text-3xl md:text-4xl font-bold text-white mb-3">
              ...
              >
                <CardHeader className="pb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <CardTitle className="text-white text-sm md:text-base">{t('appointments.status.all')}</CardTitle>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="text-3xl md:text-4xl font-bold text-white mb-3">
            ...
            <div className="flex items-center justify-between p-2 md:p-4 border-b">
              <div className="flex flex-col gap-1">
                ...
                <p className="text-xs text-muted-foreground flex items-center gap-1">
                  ...
                  <span>{t('appointments.dragToReschedule')}</span>
                </p>
              </div>
...
💡 Drag to reschedule`}
            ...
            <Form {...form}>
              <form onSubmit={form.handleSubmit((data) => {
                ...
                <Button type="submit" className="w-full" disabled={updateAppointmentMutation.isPending}>
                  {updateAppointmentMutation.isPending ? t('appointments.button.updating') : t('appointments.button.update')}
                </Button>
              </form>
            </Form>
          </DialogContent>
          ...
          <DialogHeader>
            <DialogTitle>
              {filteredStatus === null
                ...
                : `${filteredStatus.charAt(0).toUpperCase() + filteredStatus.slice(1)} ${t('appointments.title')}`}
            </DialogTitle>
          </DialogHeader>
        ...
        <DialogContent className="sm:max-w-md">
          ...
          {rescheduleTarget && (
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium">{t('appointments.dateTime')}</label>
                <div className="flex items-center gap-2">
                  <Select
      ...
      <Dialog open={showDayAppointments} onOpenChange={setShowDayAppointments}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {t('appointments.title')} - {selectedDate && format(selectedDate, 'MMMM d, yyyy')}
            </DialogTitle>
          </DialogHeader>
          <div className="max-h-[60vh] overflow-y-auto">
