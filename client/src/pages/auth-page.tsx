                ...
                <TabsContent value="login">
                  <Form {...loginForm}>
                    <form onSubmit={loginForm.handleSubmit(onLoginSubmit)} className="space-y-4">
                      ...
                      {/* Secure Demo Login Button */}
                      {allowBypassLogin && (
                        <Button 
                          type="button" 
                          variant="outline"
                          className="w-full mt-2 bg-green-500 hover:bg-green-600 text-white border-green-600 disabled:opacity-50"
                          onClick={() => {
                            demoLoginMutation.mutate({ role: 'administrator' });
                          }}
                          disabled={demoLoginMutation.isPending}
                        >
                          {demoLoginMutation.isPending ? "🔄 Creating Demo Session..." : "🚀 Demo Login (Secure)"}
                        </Button>
                      )}
