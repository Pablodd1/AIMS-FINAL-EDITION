...
export default function Notes() {
  ...
  const saveCustomPromptMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Also invalidate the system-wide query for current user's prompts
  ...
  const handleDownloadWord = async () => {
    ...
    try {
      ...
      toast({ 
        ...
        description: "Failed to generate Word document. Please try again or copy the note manually.", 
        variant: "destructive" 
      });
    } finally {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const subheader = `${NOTE_TYPES.find(t => t.id === selectedNoteType)?.name || 'SOAP Note'} • ${new Date().toLocaleDateString('en-US')}`;
      ...
      if (selectedPatient) {
        ...
        doc.text(`Name: ${selectedPatient.firstName} ${selectedPatient.lastName || ''}`, margin, yPos);
        yPos += 8;
        doc.text(`Age: ${calculateAge(selectedPatient.dateOfBirth)}`, margin, yPos);
        yPos += 15;
      }
      ...
      doc.setFontSize(14);
      doc.setFont("helvetica", "bold");
      doc.text("Medical Note Content", margin, yPos);
      yPos += 10;
      ...
      const contentLines = doc.splitTextToSize(noteText, maxWidth);
      ...
      doc.save(`medical-note-${new Date().toISOString().split('T')[0]}.pdf`);
      toast({ title: "Downloaded", description: "Note downloaded as PDF document" });
    } catch (error) {
      console.error('PDF export error:', error);
      toast({ 
        title: "PDF Export Failed", 
        description: "Failed to generate PDF document. Please try again or use the print preview.", 
        variant: "destructive" 
      });
    } finally {
  ...
  // Effects
  useEffect(() => {
    window.startRecording = () => setShowConsultationModal(true);
    window.saveNotes = handleSaveNote;
    window.previewNotes = () => setShowPreview(true);
    window.downloadNotes = handleDownloadWord;
    window.downloadPdfNotes = handleDownloadPDF;
    window.clearForm = resetWorkflow;

    return () => {
      window.startRecording = undefined;
      window.saveNotes = undefined;
      window.previewNotes = undefined;
  ...
  const handleSendChatMessage = async () => {
    ...
    try {
      const messages = [
        { role: 'system', content: 'You are the **Doctor Helper Smart** — an elite Board-Certified clinical AI assistant. Provide hospital-grade, evidence-based information. Never mention model names or being an AI. Act as a senior consultant to the provider. Always emphasize that final judgment resides with the doctor.' },
  ...
  const renderPatientSelection = () => (
    <div className="space-y-6">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold">{t('notes.step1.title')}</h2>
        <p className="text-muted-foreground">{language === 'es' ? 'Elija un paciente para crear una nota médica' : 'Choose a patient to create a medical note'}</p>
      </div>
        ...
        <CardContent>
          <Popover open={patientSearchOpen} onOpenChange={setPatientSearchOpen}>
            ...
            <PopoverContent className="w-full p-0" align="start">
              <Command>
                <CommandInput placeholder={t('notes.searchPatients')} />
  ...
  const renderPatientSummary = () => (
    <div className="space-y-6">
      <div className="text-center mb-8">
        <h2 className="text-2xl font-bold">{t('notes.step2.title')}</h2>
        <p className="text-muted-foreground">{language === 'es' ? `Elija el tipo de nota médica para crear para ${selectedPatient?.firstName}` : `Choose the type of medical note to create for ${selectedPatient?.firstName}`}</p>
      </div>
  ...
  const renderConsultation = () => (
    <div className="space-y-6">
      ...
      {/* Embedded Training RAG: Patient Consultation Context */}
      <PatientContextPanel patientId={selectedPatientId} />
            ...
            <CardContent className="flex flex-col h-[calc(100%-60px)]">
              <Tabs defaultValue="assistant" className="h-full flex flex-col">
                <TabsList className="mb-4 grid grid-cols-2 w-full">
                  ...
                  <TabsTrigger value="assistant" className="text-xs sm:text-sm">{t('notes.chat')}</TabsTrigger>
                </TabsList>
  ...
  const renderReviewSign = () => (
    <div className="space-y-6">
      <div className="text-center mb-4">
        <h2 className="text-2xl font-bold">{t('notes.step4.title')}</h2>
        <p className="text-muted-foreground">{language === 'es' ? 'Revise su nota y agregue firma electrónica' : 'Review your note and add electronic signature'}</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          <Card>
            <CardHeader>
              <CardTitle>{t('notes.electronicSignature')}</CardTitle>
              <CardDescription>{language === 'es' ? 'Firme para finalizar esta nota médica' : 'Sign to finalize this medical note'}</CardDescription>
            </CardHeader>
            <CardContent>
              {signatureData ? (
                <div className="space-y-4">
        ...
        <div className="space-y-4">
          <Card>
            <CardHeader>
              <CardTitle className="text-base">{language === 'es' ? 'Acciones' : 'Actions'}</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              <Button variant="outline" className="w-full" onClick={handleDownloadWord} disabled={isDownloading}>
                {isDownloading ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Download className="mr-2 h-4 w-4" />}
                {t('notes.downloadWord')}
              </Button>
              <Button variant="outline" className="w-full" onClick={handleDownloadPDF} disabled={isDownloadingPdf}>
      ...
      <Dialog open={showNoteSuccess} onOpenChange={setShowNoteSuccess}>
        <DialogContent>
          ...
          <p>{language === 'es' ? `La nota médica ha sido guardada en el registro de ${selectedPatient?.firstName}.` : `The medical note has been saved to ${selectedPatient?.firstName}'s record.`}</p>
          <DialogFooter>
            <Button variant="outline" onClick={resetWorkflow}>{language === 'es' ? 'Crear Otra Nota' : 'Create Another Note'}</Button>
            <Button onClick={() => setShowNoteSuccess(false)}>{language === 'es' ? 'Listo' : 'Done'}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
