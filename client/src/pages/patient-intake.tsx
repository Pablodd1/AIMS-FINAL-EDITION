...
export default function PatientIntakeFormPage() {
  ...
  React.useEffect(() => {
    window.startIntake = () => setCreateDialogOpen(true);

    return () => {
      window.startIntake = undefined;
    };
  }, []);
    ...
    mutationFn: async (data: CreateIntakeFormData) => {
      try {
        ...
        const res = await apiRequest("POST", "/api/intake-forms", {
          ...
          doctorId: 0, // Server overrides this with the authenticated user's ID
          uniqueLink,
    ...
    switch (status) {
      case "pending":
        return {
          ...
          icon: Clock,
          label: t('intake.status.pending')
        };
      case "completed":
        return {
          color: "bg-green-100 text-green-800 border-green-300",
          icon: CheckCircle,
          label: t('intake.status.completed')
        };
      case "expired":
        return {
          color: "bg-red-100 text-red-800 border-red-300",
          icon: XCircle,
          label: t('intake.status.expired')
        };
      default:
        return {
  ...
  return (
    <div className="container mx-auto p-4 space-y-6">
        ...
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {t('intake.linkDialogTitle')}
            </DialogTitle>
            <DialogDescription>
              {t('intake.linkDialogDesc')} {currentFormName} to complete their intake form
            </DialogDescription>
          </DialogHeader>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
        <div>
          <h1 className="text-3xl font-bold">{t('intake.title')}</h1>
        ...
        <div className="flex items-center gap-4">
          ...
          <Dialog open={createDialogOpen} onOpenChange={setCreateDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                <DialogTitle>{t('intake.new')}</DialogTitle>
                <DialogDescription>
                  {t('intake.newDesc')}
                </DialogDescription>
              </DialogHeader>
              <Form {...form}>
                <form onSubmit={handleSubmit} className="space-y-4">
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('intake.patient')}</FormLabel>
                        <Select
                          value={field.value}
                          onValueChange={(value) => {
                        ...
                        >
                          <FormControl>
                            <SelectTrigger>
                              <SelectValue placeholder={t('intake.selectPatient')} />
                            </SelectTrigger>
                          </FormControl>
                          <SelectContent>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('intake.fullName')}</FormLabel>
                        <FormControl>
                          <Input {...field} />
                        </FormControl>
                        <FormMessage />
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>{t('intake.email')}</FormLabel>
                        <FormControl>
                  ...
                  <DialogFooter>
                    ...
                    >
                      ...
                      {createIntakeFormMutation.isPending ? t('intake.creating') : t('intake.create')}
                    </Button>
                  </DialogFooter>
                </form>
              </Form>
      ...
      {formsLoading ? (
        ...
        <Card>
          <CardContent className="p-0">
            <Table>
              <TableCaption>{t('intake.tableCaption')}</TableCaption>
              <TableHeader>
                <TableRow>
                  <TableHead>{t('intake.table.patient')}</TableHead>
                  <TableHead>{t('intake.table.email')}</TableHead>
                  <TableHead>{t('intake.table.status')}</TableHead>
                  <TableHead>{t('intake.table.created')}</TableHead>
                  <TableHead>{t('intake.table.expires')}</TableHead>
                  <TableHead>{t('intake.table.link')}</TableHead>
                  <TableHead>{t('intake.table.actions')}</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-10">
            <div className="text-center mb-4">
              <h3 className="text-lg font-semibold">{t('intake.noForms')}</h3>
              <p className="text-muted-foreground">
                {t('intake.noFormsDesc')}
              </p>
            </div>
            <Button
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              ...
              {t('intake.resultsTitle')}: {formResults?.name}
            </DialogTitle>
            <DialogDescription>
              {t('intake.submittedOn')} {formResults?.completedAt ? format(new Date(formResults.completedAt), "PPP p") : "N/A"}
            </DialogDescription>
          </DialogHeader>
              ...
              {formResults?.responses?.find((r: any) => r.answerType === 'ai_summary') && (
                <Card className="bg-medical-dark-blue/5 border-medical-dark-blue/20">
                  <CardHeader className="py-3 px-4 flex flex-row items-center gap-2">
                    ...
                    <CardTitle className="text-sm font-bold uppercase text-medical-dark-blue">{t('intake.aiSummary')}</CardTitle>
                  </CardHeader>
              ...
              {formResults?.responses?.find((r: any) => r.answerType === 'transcript') && (
                <div className="space-y-2 pt-4 border-t">
                  <Label className="text-sm font-bold flex items-center gap-2">
                    <Mic className="h-4 w-4" /> {t('intake.transcript')}
                  </Label>
