...
export default function Patients() {
  ...
  const { data, isLoading } = useQuery<{ data: Patient[], total: number } | Patient[]>({
    queryKey: ["/api/patients", { limit: pageSize, offset: (page - 1) * pageSize }],
  ...
  const total = Array.isArray(data) ? patients.length : data?.total || 0;
  ...
  const todayPatients = patients?.filter((_: Patient, index: number) => {
  ...
  const exportPatients = () => {
    // For export, we might want all patients. Since we are paginated, 
    // we should ideally fetch all for the export or export current page.
    // For now, let's export what we have in the current view or show a toast.
    const patientsToExport = patients;
    ...
    link.setAttribute('download', `patients_export_${today}_page_${page}.csv`);
    ...
    toast({
      ...
      description: `Page ${page} data exported successfully`,
    });
  };
...
const filteredPatients = patients?.filter((patient: Patient, index: number) => {
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between border rounded-lg p-4 bg-white shadow-sm">
          <div className="text-sm text-muted-foreground">
            Showing <span className="font-medium">{(page - 1) * pageSize + 1}</span> to <span className="font-medium">{Math.min(page * pageSize, total)}</span> of <span className="font-medium">{total}</span> patients
          </div>
