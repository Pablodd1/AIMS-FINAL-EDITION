...
function VideoConsultation({ roomId, patient, onClose }: VideoConsultationProps) {
  ...
  // Pre-visit summary query
  const { data: preVisitSummary, isLoading: preVisitLoading } = useQuery({
    queryKey: ['/api/pre-visit', patient.id],
    queryFn: () => apiRequest('GET', `/api/pre-visit/${patient.id}`),
    enabled: !!patient.id,
  ...
  // Implement voice command callbacks
  useEffect(() => {
    window.startRecording = startRecording;
    window.stopRecording = stopRecording;
    window.endVideoCall = handleEndCall;

    return () => {
      window.startRecording = undefined;
      window.stopRecording = undefined;
      window.endVideoCall = undefined;
    };
          ...
          <Dialog>
            <DialogTrigger asChild>
              <Button variant="outline" size="sm" className="ml-2 hidden sm:flex">
                ...
                <span>{language === 'es' ? 'Compartir Info' : 'Share Info'}</span>
              </Button>
            </DialogTrigger>
            <DialogContent className="sm:max-w-md">
              <DialogHeader>
                <DialogTitle>{t('tele.shareTitle')}</DialogTitle>
                <DialogDescription>
                  {t('tele.shareDesc')}
                </DialogDescription>
              </DialogHeader>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        toast({
                          title: language === 'es' ? 'URL Copiada' : "URL Copied",
                          description: language === 'es' ? 'URL de unión del paciente copiada al portapapeles' : "Patient join URL copied to clipboard",
                        });
                      }}
                    >
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        toast({
                          title: language === 'es' ? 'ID Copiado' : "ID Copied",
                          description: language === 'es' ? 'ID de la sala copiado al portapapeles' : "Room ID copied to clipboard",
                        });
                      }}
                    >
                      {language === 'es' ? 'Copiar' : 'Copy'}
          ...
          <Dialog>
            ...
            <DialogContent className="w-[90vw] sm:max-w-md">
              <DialogHeader>
                <DialogTitle>{t('tele.shareTitle')}</DialogTitle>
                <DialogDescription>
                  {language === 'es' ? 'Comparta este enlace con su paciente' : 'Share this link with your patient'}
                </DialogDescription>
              </DialogHeader>
              <div className="py-4">
                <div className="space-y-2">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        toast({
                          title: language === 'es' ? 'URL Copiada' : "URL Copied",
                          description: language === 'es' ? 'URL copiada al portapapeles' : "URL copied to clipboard",
                        });
                      }}
                    >
                      {language === 'es' ? 'Copiar' : 'Copy'}
          ...
          <div className="p-2 bg-background">
            <Sheet>
                ...
                {transcription ? (
                  <SheetTrigger asChild>
                    <Button variant="outline" className="mr-auto" size="sm">
                      ...
                      <span className="hidden sm:inline">{t('tele.viewTranscript')}</span>
                      <span className="sm:hidden inline">{t('tele.transcript')}</span>
                    </Button>
                  </SheetTrigger>
                ) : (
              ...
              <SheetContent side="right" className="w-[90vw] sm:w-[540px] max-w-[95vw]">
                <SheetHeader>
                  <SheetTitle>{language === 'es' ? 'Transcripción de la Consulta' : 'Consultation Transcription'}</SheetTitle>
                  <SheetDescription>
                    {language === 'es' ? 'Transcripción generada de la consulta grabada' : 'Generated transcript from the recorded consultation'}
                  </SheetDescription>
                </SheetHeader>
            ...
            <div className="absolute bottom-0 left-0 right-0 flex flex-col sm:flex-row items-center justify-between px-2 sm:px-4 py-2 sm:py-3 bg-gradient-to-t from-black/70 to-transparent gap-2">
              <div className="flex items-center gap-2 mb-2 sm:mb-0">
                ...
                >
                  ...
                  {liveTranscriptionOpen ? t('tele.hideTranscript') : t('tele.showTranscript')}
                </Button>
                {transcription ? (
                  <Button
                    variant="outline"
              ...
              {/* Pre-Visit Summary Alert */}
              {preVisitLoading ? (
                <div className="flex items-center gap-2 text-sm text-muted-foreground">
                  Loading patient summary...
                </div>
              ) : preVisitSummary?.allergyWarning ? (
                <div className="flex items-center gap-2 text-sm bg-red-500 text-white px-2 py-1 rounded">
              ...
              <Sheet>
                <SheetTrigger asChild>
                  <Button 
                    variant="outline" 
                    size="sm" 
                    className="fixed right-2 top-2 z-10 bg-white shadow-lg border-2 border-blue-500"
                  >
                    <ClipboardList className="h-4 w-4 mr-2 text-blue-600" />
                    <span className="text-blue-600 font-medium">Patient Summary</span>
                  </Button>
                </SheetTrigger>
                <SheetContent side="left" className="w-[90vw] sm:w-[450px] max-w-[95vw]">
                  <SheetHeader>
                    <div className="flex items-center gap-2">
                      <Stethoscope className="h-5 w-5 text-blue-600" />
                      <SheetTitle>Patient Summary for {preVisitSummary.patient?.name}</SheetTitle>
                    </div>
                    <SheetDescription>
                      Pre-visit information for quick review
                    </SheetDescription>
                  </SheetHeader>
                  ...
                  <ScrollArea className="h-[calc(100vh-200px)] my-4">
                    <div className="space-y-4">
                      ...
                      {preVisitSummary.chiefComplaint && (
                        <Card>
                          <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-medium text-blue-600">Chief Complaint</CardTitle>
                          </CardHeader>
                          <CardContent>
                            <p className="text-lg">{preVisitSummary.chiefComplaint}</p>
                            {preVisitSummary.appointmentDate && (
                              <p className="text-sm text-muted-foreground">
                                Appointment: {new Date(preVisitSummary.appointmentDate).toLocaleDateString()}
                              </p>
                            )}
                          </CardContent>
                        </Card>
                      ...
                      <Card className="bg-gradient-to-r from-blue-50 to-indigo-50 border-blue-200">
                        <CardHeader className="pb-2">
                          <CardTitle className="text-sm font-medium text-blue-600">Quick Summary</CardTitle>
                        </CardHeader>
                        <CardContent className="space-y-2">
                          {preVisitSummary.quickSummary?.conditions?.length > 0 && (
                            <div className="flex items-start gap-2">
                              ...
                              <div>
                                ...
                                <p className="text-sm">{preVisitSummary.quickSummary.conditions.join(', ')}</p>
                              </div>
                            </div>
                          )}
                          {preVisitSummary.quickSummary?.allergies?.length > 0 && (
                            <div className="flex items-start gap-2">
                              <AlertCircle className="h-4 w-4 text-orange-500 mt-1 flex-shrink-0" />
                              <div>
                                <p className="text-xs text-muted-foreground">Allergies:</p>
                                <p className="text-sm text-orange-600">{preVisitSummary.quickSummary.allergies.join(', ')}</p>
                              </div>
                            </div>
                          )}
                          {preVisitSummary.quickSummary?.lastVitals && (
                            <div className="flex items-start gap-2">
                              <Stethoscope className="h-4 w-4 text-green-500 mt-1 flex-shrink-0" />
                              <div>
                                <p className="text-xs text-muted-foreground">Latest Vitals:</p>
                                <p className="text-sm">
                                  {preVisitSummary.quickSummary.lastVitals.bp && `BP: ${preVisitSummary.quickSummary.lastVitals.bp}`}
                                  {preVisitSummary.quickSummary.lastVitals.pulse && ` | Pulse: ${preVisitSummary.quickSummary.lastVitals.pulse} bpm`}
                                  {preVisitSummary.quickSummary.lastVitals.glucose && ` | Glucose: ${preVisitSummary.quickSummary.lastVitals.glucose} mg/dL`}
                                </p>
                              </div>
                            </div>
                          )}
                      ...
                      {/* Active Medications */}
                      {preVisitSummary.activeMedications?.length > 0 && (
                        <Card>
                          <CardHeader className="pb-2">
                          ...
                          <CardContent>
                            <ul className="space-y-1 text-sm">
                              {preVisitSummary.activeMedications.map((med, i) => (
                      ...
                      {/* Recent Visits */}
                      {preVisitSummary.recentVisits?.length > 0 && (
                        <Card>
                          <CardHeader className="pb-2">
                            <CardTitle className="text-sm font-medium">
                          ...
                          <CardContent>
                            <ul className="space-y-2 text-sm">
                              {preVisitSummary.recentVisits.map((visit) => (
                                <li key={visit.id} className="border-l-2 border-gray-200 pl-3">
          ...
          <div className="border rounded-lg p-4 bg-muted/20">
            ...
            {isLoading ? (
              ...
              <div className="flex items-center justify-center h-16 text-muted-foreground">
                <p>{language === 'es' ? 'No hay grabaciones disponibles para esta consulta' : 'No recording available for this consultation'}</p>
              </div>
            )}
          </div>
...
export default function Telemedicine() {
  ...
  // Implement voice command callbacks
  useEffect(() => {
    window.startVideoCall = () => setShowStartDialog(true);

    return () => {
      window.startVideoCall = undefined;
    };
  }, []);
      ...
      {activeConsultation ? (
        ...
        <div className="space-y-4 md:space-y-6 p-4 md:p-0">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-bold">{t('tele.title')}</h1>
              <p className="text-sm md:text-base text-muted-foreground">{language === 'es' ? 'Gestione sus consultas virtuales' : 'Manage virtual consultations'}</p>
            </div>
          ...
          <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">{t('dashboard.stats.upcoming')}</CardTitle>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">{language === 'es' ? 'Sesiones de telemedicina programadas' : 'Scheduled telemedicine sessions'}</p>
              </CardContent>
            </Card>

            <Card>
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                <CardTitle className="text-sm font-medium">{t('dashboard.stats.appointments')}</CardTitle>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">{language === 'es' ? 'Citas para hoy' : 'Appointments for today'}</p>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent>
                ...
                <p className="text-xs text-muted-foreground">{language === 'es' ? 'Consultas completadas totales' : 'Total completed consultations'}</p>
              </CardContent>
            </Card>
          </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle>{t('tele.waitingRoom')}</CardTitle>
            </CardHeader>
            <CardContent>
