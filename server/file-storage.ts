...
export class FileStorage {
  ...
  static async saveRecording(
    ...
    // Sanitize extension to prevent path traversal or unauthorized file types
    const sanitizedExtension = extension.replace(/[^a-z0-9]/gi, '').toLowerCase();
    const allowedExtensions = ['webm', 'mp4', 'ogg', 'wav', 'm4a', 'aac'];

    if (!allowedExtensions.includes(sanitizedExtension)) {
      throw new Error(`Unauthorized file extension: ${sanitizedExtension}`);
    }
    
    const filename = `${recordingId}_${mediaType}.${sanitizedExtension}`;
