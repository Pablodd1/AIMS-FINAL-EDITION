...
(async () => {
  ...
  // ==========================================
  // AUTO-SETUP: Initialize default users if none exist
  // This prevents login failures after first deployment
  // ==========================================
  try {
    ...
    if (users.length === 0) {
      log('ℹ️ No users found in database. Initializing default accounts...');
      ...
      for (const userData of defaultUsers) {
        ...
        log(`✅ Created default user: ${userData.username}`);
      }
    }
  } catch (error) {
    logError('Warning: Auto-setup check failed (app will continue):', error);
  }
