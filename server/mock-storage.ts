...
export class MockStorage implements IStorage {
    ...
    private cptCodes: Map<number, CptCode> = new Map();
    private icd10Codes: Map<number, Icd10Code> = new Map();
    private favoriteCodes: Map<number, FavoriteCode> = new Map();
    private medicalNoteCoding: Map<number, MedicalNoteCoding> = new Map();
    ...
    private saveData() {
        const data = {
            ...
            cptCodes: Array.from(this.cptCodes.entries()),
            icd10Codes: Array.from(this.icd10Codes.entries()),
            favoriteCodes: Array.from(this.favoriteCodes.entries()),
            medicalNoteCoding: Array.from(this.medicalNoteCoding.entries()),
    ...
    private loadData(): boolean {
        try {
            if (fs.existsSync(PERSISTENCE_FILE)) {
                ...
                this.cptCodes = new Map(data.cptCodes || []);
                this.icd10Codes = new Map(data.icd10Codes || []);
                this.favoriteCodes = new Map(data.favoriteCodes || []);
                this.medicalNoteCoding = new Map(data.medicalNoteCoding || []);
    ...
    async createUser(user: InsertUser): Promise<User> {
        ...
        const newUser: User = {
            ...
            clinicLocation: (user as any).clinicLocation ?? null,
        };
    ...
    async getPatients(doctorId: number, limit?: number, offset?: number): Promise<Patient[]> {
        let results = Array.from(this.patients.values()).filter(p => p.createdBy === doctorId);
        results.sort((a, b) => b.id - a.id); // Descending order
        if (offset !== undefined) results = results.slice(offset);
        if (limit !== undefined) results = results.slice(0, limit);
        return results;
    }
    async getPatientsCount(doctorId: number): Promise<number> {
        return Array.from(this.patients.values()).filter(p => p.createdBy === doctorId).length;
    }
    async getClinicPatients(location: string, limit?: number, offset?: number): Promise<Patient[]> {
        ...
        let results = Array.from(this.patients.values()).filter(p => doctorIds.includes(p.createdBy));
    ...
    async getClinicPatientsCount(location: string): Promise<number> {
        const doctors = Array.from(this.users.values()).filter(u => u.clinicLocation === location);
        const doctorIds = doctors.map(d => d.id);
        return Array.from(this.patients.values()).filter(p => doctorIds.includes(p.createdBy)).length;
    }
    ...
    async checkExistingPatient(email: string, phone: string): Promise<Patient[]> {
        return Array.from(this.patients.values()).filter(p => p.email === email || p.phone === phone);
    }
    ...
    async updatePatientCheckIn(patientId: number, checkInData: any): Promise<Patient> {
        ...
        await this.createPatientActivity({
            ...
            description: `Checked in via kiosk`,
            createdBy: 1,
    ...
    async getWaitingRoomData(): Promise<any[]> {
        const activities = Array.from(this.patientActivity.values())
            .filter(a => a.activityType === 'appointment' && a.title === 'Patient Checked In')
            .sort((a, b) => {
        ...
        for (const activity of activities) {
            if (!processedPatients.has(activity.patientId)) {
                const patient = this.patients.get(activity.patientId);
                if (patient) {
                    waitingPatients.push({
                        ...patient,
                        checkInTime: activity.date,
                        status: (activity.metadata as any)?.status || 'waiting',
                        appointmentType: (activity.metadata as any)?.appointmentType || 'In-person',
                        estimatedWaitTime: 15 // Placeholder
                    });
                }
                processedPatients.add(activity.patientId);
            }
        }
    ...
    async getTodayAppointments(): Promise<any[]> {
        ...
        return this.getAppointmentsForDateRange(todayStart, todayEnd);
    }
    ...
    async createAppointment(appointment: InsertAppointment): Promise<Appointment> {
        ...
        const newAppointment: Appointment = {
            ...
            confirmationToken: (appointment as any).confirmationToken ?? null,
    ...
    async getCptCodes(doctorId: number): Promise<CptCode[]> {
        return Array.from(this.cptCodes.values()).filter(c => c.doctorId === doctorId).sort((a, b) => a.code.localeCompare(b.code));
    }
    async getCptCode(id: number): Promise<CptCode | undefined> {
        return this.cptCodes.get(id);
    }
    async getCptCodeByCode(code: string, doctorId: number): Promise<CptCode | undefined> {
        return Array.from(this.cptCodes.values()).find(c => c.code === code && c.doctorId === doctorId);
    }
    async createCptCode(code: InsertCptCode): Promise<CptCode> {
        const id = this.currentId++;
        const newCode: CptCode = { ...code, id, modifier: code.modifier ?? null, rvu: code.rvu ?? null, isCustom: code.isCustom ?? true, createdAt: new Date(), updatedAt: new Date() };
        this.cptCodes.set(id, newCode);
        this.saveData();
        return newCode;
    }
    async updateCptCode(id: number, updates: Partial<CptCode>): Promise<CptCode | undefined> {
        const code = this.cptCodes.get(id);
        if (!code) return undefined;
        const updated = { ...code, ...updates, updatedAt: new Date() };
        this.cptCodes.set(id, updated);
        this.saveData();
        return updated;
    }
    async deleteCptCode(id: number): Promise<void> {
        this.cptCodes.delete(id);
        this.saveData();
    }

    // ICD-10 Codes methods
    async getIcd10Codes(doctorId: number): Promise<Icd10Code[]> {
        return Array.from(this.icd10Codes.values()).filter(c => c.doctorId === doctorId).sort((a, b) => a.code.localeCompare(b.code));
    }
    async getIcd10Code(id: number): Promise<Icd10Code | undefined> {
        return this.icd10Codes.get(id);
    }
    async getIcd10CodeByCode(code: string, doctorId: number): Promise<Icd10Code | undefined> {
        return Array.from(this.icd10Codes.values()).find(c => c.code === code && c.doctorId === doctorId);
    }
    async createIcd10Code(code: InsertIcd10Code): Promise<Icd10Code> {
        const id = this.currentId++;
        const newCode: Icd10Code = { ...code, id, category: code.category ?? 'General', laterality: code.laterality ?? null, isCustom: code.isCustom ?? true, createdAt: new Date(), updatedAt: new Date() };
        this.icd10Codes.set(id, newCode);
        this.saveData();
        return newCode;
    }
    async updateIcd10Code(id: number, updates: Partial<Icd10Code>): Promise<Icd10Code | undefined> {
        const code = this.icd10Codes.get(id);
        if (!code) return undefined;
        const updated = { ...code, ...updates, updatedAt: new Date() };
        this.icd10Codes.set(id, updated);
        this.saveData();
        return updated;
    }
    async deleteIcd10Code(id: number): Promise<void> {
        this.icd10Codes.delete(id);
        this.saveData();
    }

    // Favorite Codes methods
    async getFavoriteCodes(doctorId: number): Promise<FavoriteCode[]> {
        return Array.from(this.favoriteCodes.values()).filter(f => f.doctorId === doctorId).sort((a, b) => {
    ...
    async addFavoriteCode(favorite: InsertFavoriteCode): Promise<FavoriteCode> {
        const id = this.currentId++;
        const newFavorite: FavoriteCode = { ...favorite, id, createdAt: new Date() };
        this.favoriteCodes.set(id, newFavorite);
        this.saveData();
        return newFavorite;
    }
    async removeFavoriteCode(id: number, doctorId: number): Promise<void> {
        this.favoriteCodes.delete(id);
        this.saveData();
    }

    // Medical Note Coding methods
    async getMedicalNoteCoding(noteId: number): Promise<MedicalNoteCoding[]> {
        return Array.from(this.medicalNoteCoding.values()).filter(c => c.noteId === noteId);
    }
    async createMedicalNoteCoding(coding: InsertMedicalNoteCoding): Promise<MedicalNoteCoding> {
        const id = this.currentId++;
        const newCoding: MedicalNoteCoding = {
            ...coding,
            id,
            cptCodeId: coding.cptCodeId ?? null,
            icd10CodeId: coding.icd10CodeId ?? null,
            units: coding.units ?? 1,
            modifier: coding.modifier ?? null,
            isPrimary: coding.isPrimary ?? false,
            createdAt: new Date()
        };
        this.medicalNoteCoding.set(id, newCoding);
        this.saveData();
        return newCoding;
    }
    async deleteMedicalNoteCoding(id: number): Promise<void> {
        this.medicalNoteCoding.delete(id);
        this.saveData();
    }
}
