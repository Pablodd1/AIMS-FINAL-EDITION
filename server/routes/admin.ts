...
adminRouter.post('/global-api-key', async (req: Request, res: Response) => {
  try {
    ...
    // Basic validation for OpenAI API key format (supports standard sk-... and sk-proj-...)
    const isValidKey = /^sk-(proj-)?[a-zA-Z0-9_-]{20,}$/.test(apiKey) && apiKey.length >= 40;
    if (!isValidKey) {
...
// Global Kimi API Key Management
adminRouter.get('/global-kimi-key', async (req: Request, res: Response) => {
  try {
    const globalApiKey = await storage.getSystemSetting('global_kimi_api_key');
    const maskedKey = globalApiKey ? `${globalApiKey.substring(0, 6)}...` : null;
    res.json({ hasApiKey: !!globalApiKey, maskedKey });
  } catch (error) {
    res.status(500).json({ error: 'Failed to fetch global Kimi API key' });
  }
});

adminRouter.post('/global-kimi-key', async (req: Request, res: Response) => {
  try {
    const { apiKey } = req.body;
    if (!apiKey || typeof apiKey !== 'string') {
      return res.status(400).json({ error: 'Valid API key is required' });
    ...
    await storage.setSystemSetting('global_kimi_api_key', apiKey, 'Global Kimi API key', userId);
    res.json({ success: true, message: 'Global Kimi API key updated successfully' });
  } catch (error) {
    res.status(500).json({ error: 'Failed to update global Kimi API key' });
  }
});

// AI Provider Management
adminRouter.get('/ai-provider', async (req: Request, res: Response) => {
  try {
    const provider = await storage.getSystemSetting('global_ai_provider') || 'openai';
    res.json({ provider });
  } catch (error) {
...
adminRouter.post('/ai-provider', async (req: Request, res: Response) => {
  try {
    ...
    const userId = req.user?.id || 1;
    await storage.setSystemSetting('global_ai_provider', provider, 'Global AI Provider Choice', userId);
    res.json({ success: true, message: `AI provider switched to ${provider}` });
  } catch (error) {
...
adminRouter.get('/global-prompts', async (req: Request, res: Response) => {
  try {
    const prompts = await storage.getGlobalPrompts();
    res.json(prompts);
  } catch (error) {
    res.json([]);
  }
...
adminRouter.get('/global-prompts/:id', async (req: Request, res: Response) => {
  try {
    ...
    const prompt = await storage.getGlobalPrompt(promptId);
    if (!prompt) {
...
adminRouter.post('/global-prompts', async (req: Request, res: Response) => {
  try {
    ...
    const prompt = await storage.createGlobalPrompt({
      userId,
      noteType: note_type,
      name,
      description,
...
adminRouter.put('/global-prompts/:id', async (req: Request, res: Response) => {
  try {
    ...
    const currentPrompt = await storage.getGlobalPrompt(promptId);
    if (!currentPrompt) {
      return res.status(404).json({ error: 'Prompt not found' });
    }

    const currentVersion = parseFloat(currentPrompt.version || "1.0") || 1.0;
    const newVersion = (currentVersion + 0.1).toFixed(1);

    const updated = await storage.updateGlobalPrompt(promptId, {
      name,
      description,
      noteType: note_type,
      systemPrompt: system_prompt,
...
adminRouter.delete('/global-prompts/:id', async (req: Request, res: Response) => {
  try {
    ...
    const deleted = await storage.deleteGlobalPrompt(promptId);
    if (!deleted) {
      return res.status(404).json({ error: 'Prompt not found' });
    }
