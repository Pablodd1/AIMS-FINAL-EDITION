...
import { SoapResponseSchema, IntakeAnswersSchema } from '../ai-schemas';
import { getAIClient } from '../ai-utils';

// Use utility instead of local getOpenAIClient
...
aiRouter.post('/chat', requireAuth, asyncHandler(async (req, res) => {
  ...
  if (!aiClient) {
    ...
    const errorMessage = user?.useOwnApiKey
      ? 'No personal AI API key found. Please add your AI API key in Settings to use AI features.'
      : 'No global AI API key configured. Please contact your administrator or add your own API key in Settings.';
...
aiRouter.post('/generate-title', async (req, res) => {
  try {
    ...
    if (!aiClient) {
      ...
      const errorMessage = user?.useOwnApiKey
        ? 'No personal AI API key found. Please add your API key in Settings to use AI features.'
        : 'No global AI API key configured. Please contact your administrator or add your own API key in Settings.';
...
aiRouter.post('/generate-soap', async (req, res) => {
  try {
    ...
    // Fetch deep patient history if ID is available (Embedded training RAG)
    let patientHistory = "";
    if (patientInfo?.id && !isNaN(parseInt(patientInfo.id))) {
      try {
        const patientId = parseInt(patientInfo.id);
        const [notes, medications, alerts, historyEntries] = await Promise.all([
          dbStorage.getMedicalNotesByPatient(patientId),
          dbStorage.getPrescriptionsByPatient(patientId),
          dbStorage.getMedicalAlertsByPatient(patientId),
          dbStorage.getMedicalHistoryEntriesByPatient(patientId)
        ]);
        ...
        if (alerts && alerts.length > 0) {
          const activeAlerts = alerts.filter(a => a.isActive).map(a =>
            `- [${a.severity.toUpperCase()}] ${a.title}: ${a.description || ''}`
          ).join('\n');
          contextParts.push(`MEDICAL ALERTS:\n${activeAlerts}`);
        }

        if (historyEntries && historyEntries.length > 0) {
          const history = historyEntries.filter(h => h.isActive).map(h =>
            `- ${h.category.toUpperCase()}: ${h.title} (${h.date ? new Date(h.date).toLocaleDateString() : 'no date'})`
          ).join('\n');
          contextParts.push(`MEDICAL HISTORY:\n${history}`);
        }

        if (contextParts.length > 0) {
          patientHistory = `\n\nPATIENT CLINICAL CONTEXT (RAG):\n${contextParts.join('\n\n')}`;
        }
      } catch (err) {
        logError('Error fetching deep patient history for context: ' + (err as Error).message);
      }
    }

    // No mock mode — require a real API key for all SOAP note generation
    ...
    try {
      ...
      if (!aiClient) {
        return res.json({
          success: false,
          soap: 'AI service not configured.'
        });
      }
...
Return the complete JSON object with ehr_payload and human_note fields.`
      ...
      try {
        ...
        const result = SoapResponseSchema.safeParse(JSON.parse(responseContent));
        
        const aimsResponse = result.success ? result.data : {
          human_note: responseContent,
          ehr_payload: {}
        };
...
aiRouter.post('/transcribe', upload.single('audio'), async (req, res) => {
  try {
    ...
    const deepgramApiKey = process.env.DEEPGRAM_API_KEY && process.env.DEEPGRAM_API_KEY !== 'YOUR_KEY_HERE' ? process.env.DEEPGRAM_API_KEY : null;
    const openaiApiKey = process.env.OPENAI_API_KEY && !['YOUR_KEY_HERE', 'sk-dummy-key'].includes(process.env.OPENAI_API_KEY) ? process.env.OPENAI_API_KEY : null;

    // No mock transcription — require a real API key
    if (!deepgramApiKey && !openaiApiKey) {
      log('No transcription API keys configured');
      return res.status(503).json({
        error: 'No transcription service configured. Please add a Deepgram or OpenAI API key in your environment or Settings.',
        provider: 'none'
      });
    }
    ...
    if (!deepgramApiKey) {
      ...
      // Try user-level API key first, then fall back to env-level key
      const userId = req.user?.id;
      ...
      // If no user-level key, try env-level key directly
      ...
      return res.status(503).json({
        error: 'Transcription service not configured. Please set DEEPGRAM_API_KEY or OPENAI_API_KEY, or use browser live transcription.',
...
aiRouter.post('/extract-intake-answers', asyncHandler(async (req, res) => {
  ...
  if (!apiKey || ['YOUR_KEY_HERE', 'sk-dummy-key', 'mock'].includes(apiKey)) {
    throw new AppError('OpenAI API not configured. Please add a valid API key.', 503, 'NO_API_KEY');
  }
  ...
  try {
    ...
    // Zod validation for intake answers
    let parsed;
    try {
      const result = IntakeAnswersSchema.safeParse(JSON.parse(content));
      parsed = result.success ? result.data : { answers: {}, summary: '' };
    } catch (e) {
      logError('Failed to parse AI intake answers: ' + (e as Error).message);
