...
import { LabAnalysisSchema } from '../ai-schemas';
...
// Use centralized getAIClient utility instead of local helper
...
labInterpreterRouter.post('/analyze', requireAuth, asyncHandler(async (req, res) => {
  ...
  let systemPrompt = settings?.system_prompt || 'You are an elite **Functional Medicine Pathologist** and **Clinical Data Analyst**. Your goal is to provide a comprehensive interpretation of medical laboratory results, blending old-school medical rigor with modern root-cause analysis. \n\n**CRITICAL GUIDELINES:**\n1. Identify both acute/chronic pathologies and sub-clinical functional imbalances.\n2. Connect biomarkers into a physiological narrative.\n3. Identify red flags for missing data or critical values.\n4. Recommend specific ICD-10 diagnoses and supporting CPT codes.\n5. Suggest relevant medications and follow-up lab panels to justify findings.\n6. Be strictly evidence-based and factual based on the provided text.';
    ...
    if (patient) {
      ...
      // Replace placeholders with actual patient info
        userPrompt = userPrompt
          .replace('${patientName}', `${patient.firstName} ${patient.lastName || ''}`.trim())
          .replace('${patientId}', patient.id.toString());
        log('Using patient-specific prompt for:', { patientName: `${patient.firstName} ${patient.lastName || ''}`.trim() });
    }
  }
...
{ 
  ...
  "interpretation": "detailed functional medicine analysis of the lab results (Old School MD + Root Cause analysis)",
  "diagnoses": [{"code": "ICD-10 code", "description": "diagnosis description"}],
  "cptCodes": [{"code": "CPT code", "description": "procedure/service description"}],
  ...
  "additionalTesting": ["suggest further labs or tests needed to justify diagnosis"],
  ...
  if (withPatient && patientId && patient) {
    await handleDatabaseOperation(
      () => storage.createLabReport({
        ...
        reportType: 'text',
        title: `Lab Report Analysis - ${patient.firstName} ${patient.lastName || ''}`.trim(),
        analysis: analysis
      }),
    ...
    if (withPatient === 'true' && patientId) {
      ...
      if (patient) {
        ...
        userPrompt = userPrompt
          .replace('${patientName}', `${patient.firstName} ${patient.lastName || ''}`.trim())
          .replace('${patientId}', patient.id.toString());
        log('Using patient-specific prompt for upload analysis: ' + `${patient.firstName} ${patient.lastName || ''}`.trim());
      }
    }
...
{ 
  ...
  "dataQuality": "assessment of extracted lab data completeness and integrity"
    ...
    // Try to parse and validate the analysis with Zod
    let parsedAnalysis = null;
    try {
      const result = LabAnalysisSchema.safeParse(JSON.parse(analysis));
      if (result.success) {
        parsedAnalysis = result.data;
        log('Analysis successfully parsed and validated as JSON');
      } else {
        logError('AI response format invalid according to schema:', result.error as any);
        // We still have the raw analysis string, so we continue
      }
    } catch (parseError) {
...
labInterpreterRouter.post('/follow-up', async (req, res) => {
  try {
    ...
    const aiClient = await getAIClient(doctorId);
    if (!aiClient) {
  ...
  try {
    ...
    if (format === 'pdf') {
      ...
      try {
        ...
        let parsed;
        try {
          const result = LabAnalysisSchema.safeParse(JSON.parse(content));
          parsed = result.success ? result.data : JSON.parse(content);
        } catch (e) {
          // If not JSON, try parsing the formatted content without HTML tags
          try {
            const cleanContent = formattedContent.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ');
            const result = LabAnalysisSchema.safeParse(JSON.parse(cleanContent));
            parsed = result.success ? result.data : JSON.parse(cleanContent);
          } catch (e2) {
    ...
    throw new AppError(`Failed to generate styled ${format.toUpperCase()} document: ${(error as Error).message}`, 500, 'DOCUMENT_GENERATION_ERROR');
  }
}));

export default labInterpreterRouter;