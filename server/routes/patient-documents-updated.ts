...
patientDocumentsRouter.get('/:patientId', async (req, res) => {
  try {
    ...
    if (patient.doctorId !== userId) {
      return res.status(403).json({ error: 'Access denied to this patient\'s documents' });
    }
...
patientDocumentsRouter.post('/:patientId/upload', upload.single('document'), async (req, res) => {
  try {
    ...
    if (patient.doctorId !== doctorId) {
      return res.status(403).json({ error: 'Access denied to upload documents for this patient' });
    }
...
patientDocumentsRouter.get('/:patientId/download/:documentId', async (req, res) => {
  try {
    ...
    const sanitizedFilename = document.originalFilename?.replace(/[^\w\.\-]/g, '_') || document.filename;
