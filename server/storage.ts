import {
  ...
  type FavoriteCode,
  type InsertFavoriteCode,
  type MedicalNoteCoding,
  type InsertMedicalNoteCoding
} from "@shared/schema";
...
export interface IStorage {
  ...
  // Kiosk methods
  checkExistingPatient(email: string, phone: string): Promise<Patient[]>;
  createPatientFromKiosk(patientData: any): Promise<Patient>;
  updatePatientCheckIn(patientId: number, checkInData: any): Promise<Patient>;
  updatePatientStatus(patientId: number, statusData: any): Promise<Patient>;
  getWaitingRoomData(): Promise<any[]>;
  getWaitingRoomCount(): Promise<number>;
  getTodayAppointments(): Promise<any[]>;
  removeCompletedPatients(): Promise<Patient[]>;
  ...
  // CPT Codes methods
  getCptCodes(doctorId: number): Promise<CptCode[]>;
  getCptCode(id: number): Promise<CptCode | undefined>;
  getCptCodeByCode(code: string, doctorId: number): Promise<CptCode | undefined>;
  createCptCode(code: InsertCptCode): Promise<CptCode>;
  updateCptCode(id: number, updates: Partial<CptCode>): Promise<CptCode | undefined>;
  deleteCptCode(id: number): Promise<void>;

  // ICD-10 Codes methods
  getIcd10Codes(doctorId: number): Promise<Icd10Code[]>;
  getIcd10Code(id: number): Promise<Icd10Code | undefined>;
  getIcd10CodeByCode(code: string, doctorId: number): Promise<Icd10Code | undefined>;
  createIcd10Code(code: InsertIcd10Code): Promise<Icd10Code>;
  updateIcd10Code(id: number, updates: Partial<Icd10Code>): Promise<Icd10Code | undefined>;
  deleteIcd10Code(id: number): Promise<void>;

  // Favorite codes methods
  getFavoriteCodes(doctorId: number): Promise<FavoriteCode[]>;
  addFavoriteCode(favorite: InsertFavoriteCode): Promise<FavoriteCode>;
  removeFavoriteCode(id: number, doctorId: number): Promise<void>;

  // Medical note coding methods
  getMedicalNoteCoding(noteId: number): Promise<MedicalNoteCoding[]>;
  createMedicalNoteCoding(coding: InsertMedicalNoteCoding): Promise<MedicalNoteCoding>;
  deleteMedicalNoteCoding(id: number): Promise<void>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getPatients(doctorId: number, limit?: number, offset?: number, isAdmin?: boolean): Promise<Patient[]> {
    let query;
    if (isAdmin) {
      query = db.select().from(patients).orderBy(sql`${patients.id} DESC`);
    } else {
      query = db.select().from(patients).where(eq(patients.createdBy, doctorId)).orderBy(sql`${patients.id} DESC`);
    }
  ...
  async getClinicPatients(location: string, limit?: number, offset?: number): Promise<Patient[]> {
    let query = db
      .select({
        patient: patients
      })
  ...
  async getClinicPatientsCount(location: string): Promise<number> {
    const [result] = await db
  ...
  async getRecordingSessions(doctorId: number): Promise<RecordingSession[]> {
    return db.select()
      ...
      .orderBy(sql`${recordingSessions.id} DESC`);
  }
  ...
  async getBpReadings(patientId: number, limit?: number): Promise<BpReading[]> {
    const query = db.select()
      ...
      .orderBy(sql`${bpReadings.id} DESC`);
  ...
  async getGlucoseReadings(patientId: number, limit?: number): Promise<GlucoseReading[]> {
    const query = db.select()
      ...
      .orderBy(sql`${glucoseReadings.id} DESC`);
    ...
    try {
      if (userId) {
        return await db.select().from(labKnowledgeBase)
          ...
          .orderBy(sql`${labKnowledgeBase.id} ASC`);
      } else {
        // For backward compatibility, return all if no userId provided
        return await db.select().from(labKnowledgeBase).orderBy(sql`${labKnowledgeBase.id} ASC`);
      }
    } catch (error) {
  ...
  async getLabReports(doctorId: number): Promise<LabReport[]> {
    try {
      return await db
        ...
        .orderBy(sql`${labReports.id} DESC`);
    } catch (error) {
  ...
  async getMedicalNoteTemplates(): Promise<MedicalNoteTemplate[]> {
    try {
      return await db
        ...
        .orderBy(sql`${medicalNoteTemplates.id} ASC`);
    } catch (error) {
  ...
  async getPatientDocuments(patientId: number): Promise<PatientDocument[]> {
    try {
      return db
        ...
        .orderBy(sql`${patientDocuments.id} DESC`);
    } catch (error) {
...
async deletePatientDocument(id: number): Promise<boolean> {
    ...
    try {
      const results = await db
        .select({
          ...
          checkInTime: patientActivity.date,
          metadata: patientActivity.metadata,
        })
        .from(patientActivity)
        .innerJoin(patients, eq(patientActivity.patientId, patients.id))
        .where(
          and(
            eq(patientActivity.activityType, 'appointment'),
            sql`${patientActivity.title} = 'Patient Checked In'`
          )
        )
        .orderBy(sql`${patientActivity.id} DESC`);

      return results.map(r => ({
        ...r,
        status: (r.metadata as any)?.status || 'waiting',
        appointmentType: (r.metadata as any)?.appointmentType || 'In-person',
        estimatedWaitTime: 15
      }));
    } catch (error) {
      logError("Error fetching waiting room data:", error);
      return [];
    }
  }

  async getWaitingRoomCount(): Promise<number> {
    const data = await this.getWaitingRoomData();
    return data.length;
  }
  ...
  async getTodayAppointments(): Promise<any[]> {
    try {
      ...
      const results = await this.getAppointmentsForDateRange(todayStart, todayEnd);
      return results;
    } catch (error) {
  ...
  async getMedicalAlertsByPatient(patientId: number): Promise<MedicalAlert[]> {
    return db.select().from(medicalAlerts)
      ...
      .orderBy(sql`${medicalAlerts.id} DESC`);
  }
  ...
  async getMedicalHistoryEntriesByPatient(patientId: number): Promise<MedicalHistoryEntry[]> {
    return db.select().from(medicalHistoryEntries)
      ...
      .orderBy(sql`${medicalHistoryEntries.id} DESC`);
  }
  ...
  // ==========================================
  // CPT Codes Implementation
  // ==========================================
  async getCptCodes(doctorId: number): Promise<CptCode[]> {
    return await db
      .select()
      .from(cptCodes)
  ...
  async getCptCode(id: number): Promise<CptCode | undefined> {
    const [code] = await db.select().from(cptCodes).where(eq(cptCodes.id, id));
    return code;
  }

  async getCptCodeByCode(code: string, doctorId: number): Promise<CptCode | undefined> {
    const [found] = await db
      .select()
      .from(cptCodes)
      .where(and(eq(cptCodes.code, code), eq(cptCodes.doctorId, doctorId)));
    return found;
  }

  async createCptCode(code: InsertCptCode): Promise<CptCode> {
    const [created] = await db.insert(cptCodes).values(code).returning();
    return created;
  }

  async updateCptCode(id: number, updates: Partial<CptCode>): Promise<CptCode | undefined> {
    const [updated] = await db
      .update(cptCodes)
  ...
  async deleteCptCode(id: number): Promise<void> {
  ...
  // ==========================================
  async getIcd10Codes(doctorId: number): Promise<Icd10Code[]> {
    return await db
      .select()
      .from(icd10Codes)
  ...
  async getIcd10CodeByCode(code: string, doctorId: number): Promise<Icd10Code | undefined> {
    const [found] = await db
      .select()
      .from(icd10Codes)
      .where(and(eq(icd10Codes.code, code), eq(icd10Codes.doctorId, doctorId)));
    return found;
  }

  async createIcd10Code(code: InsertIcd10Code): Promise<Icd10Code> {
    const [created] = await db.insert(icd10Codes).values(code).returning();
    return created;
  }

  async updateIcd10Code(id: number, updates: Partial<Icd10Code>): Promise<Icd10Code | undefined> {
    const [updated] = await db
      .update(icd10Codes)
      .set({ ...updates, updatedAt: new Date() })
  ...
  async deleteIcd10Code(id: number): Promise<void> {
    await db.delete(icd10Codes).where(eq(icd10Codes.id, id));
  }

  // ==========================================
  // Favorite Codes Implementation
  // ==========================================
  async getFavoriteCodes(doctorId: number): Promise<FavoriteCode[]> {
    return await db
      .select()
      .from(favoriteCodes)
      .where(eq(favoriteCodes.doctorId, doctorId))
      .orderBy(sql`${favoriteCodes.id} DESC`);
  }

  async addFavoriteCode(favorite: InsertFavoriteCode): Promise<FavoriteCode> {
    const [created] = await db.insert(favoriteCodes).values(favorite).returning();
    return created;
  }

  async removeFavoriteCode(id: number, doctorId: number): Promise<void> {
    await db
      .delete(favoriteCodes)
      .where(and(eq(favoriteCodes.id, id), eq(favoriteCodes.doctorId, doctorId)));
  }

  // ==========================================
  // Medical Note Coding Implementation
  // ==========================================
  async getMedicalNoteCoding(noteId: number): Promise<MedicalNoteCoding[]> {
    return await db
      .select()
      .from(medicalNoteCoding)
      .where(eq(medicalNoteCoding.noteId, noteId));
  }

  async createMedicalNoteCoding(coding: InsertMedicalNoteCoding): Promise<MedicalNoteCoding> {
    const [created] = await db.insert(medicalNoteCoding).values(coding).returning();
    return created;
  }

  async deleteMedicalNoteCoding(id: number): Promise<void> {
    await db.delete(medicalNoteCoding).where(eq(medicalNoteCoding.id, id));
  }
}

// Create and export a singleton instance
// Use DatabaseStorage if DATABASE_URL is set, otherwise fallback to MockStorage for local development/demo
export const storage = process.env.DATABASE_URL ? new DatabaseStorage() : new MockStorage();

if (!process.env.DATABASE_URL) {
  log('Using MockStorage as DATABASE_URL is not set');
}
