...
export const appointments = pgTable("appointments", {
  ...
  patientConfirmationStatus: text("patient_confirmation_status").notNull().default("pending_confirmation"),
...
// ==========================================
// CPT Codes Table - For billing procedures
// ==========================================
export const cptCodes = pgTable("cpt_codes", {
  id: serial("id").primaryKey(),
  doctorId: integer("doctor_id").references(() => users.id).notNull(),
  code: text("code").notNull(), // e.g., "99213"
  description: text("description").notNull(),
  category: text("category").default("General"), // e.g., "E/M", "Procedure", "Lab"
  modifier: text("modifier"), // e.g., "25", "59"
  rvu: real("rvu"), // Relative Value Unit
  isCustom: boolean("is_custom").default(true), // true = user added, false = system default
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

export const insertCptCodeSchema = createInsertSchema(cptCodes).pick({
  doctorId: true,
  code: true,
  description: true,
  category: true,
...
export type InsertCptCode = z.infer<typeof insertCptCodeSchema>;
...
export const icd10Codes = pgTable("icd10_codes", {
  id: serial("id").primaryKey(),
  doctorId: integer("doctor_id").references(() => users.id).notNull(),
  code: text("code").notNull(), // e.g., "I10"
  description: text("description").notNull(),
  category: text("category").default("General"), // e.g., "Cardiovascular", "Respiratory"
  laterality: text("laterality"), // e.g., "right", "left", "bilateral"
  isCustom: boolean("is_custom").default(true),
  createdAt: timestamp("created_at").defaultNow(),
  updatedAt: timestamp("updated_at").defaultNow(),
});

export const insertIcd10CodeSchema = createInsertSchema(icd10Codes).pick({
  doctorId: true,
  code: true,
  description: true,
  category: true,
...
// ==========================================
// Favorite Codes Table - User's quick access codes
// ==========================================
export const favoriteCodes = pgTable("favorite_codes", {
  id: serial("id").primaryKey(),
  doctorId: integer("doctor_id").references(() => users.id).notNull(),
  codeId: integer("code_id").notNull(),
  codeType: text("code_type").notNull(), // 'cpt' or 'icd10'
  createdAt: timestamp("created_at").defaultNow(),
});

export const insertFavoriteCodeSchema = createInsertSchema(favoriteCodes).pick({
  doctorId: true,
  codeId: true,
  codeType: true,
});

export type FavoriteCode = typeof favoriteCodes.$inferSelect;
export type InsertFavoriteCode = z.infer<typeof insertFavoriteCodeSchema>;

// ==========================================
// Medical Note Coding - Link notes to CPT/DX codes
// ==========================================
export const medicalNoteCoding = pgTable("medical_note_coding", {
  id: serial("id").primaryKey(),
  noteId: integer("note_id").references(() => medicalNotes.id, { onDelete: "cascade" }).notNull(),
...
export const insertMedicalNoteCodingSchema = createInsertSchema(medicalNoteCoding).pick({
  noteId: true,
  cptCodeId: true,
  icd10CodeId: true,
  units: true,
...
export type MedicalNoteCoding = typeof medicalNoteCoding.$inferSelect;
export type InsertMedicalNoteCoding = z.infer<typeof insertMedicalNoteCodingSchema>;
...
export const apiKeys = pgTable("api_keys", {
  ...
  service: text("service").notNull(), // 'deepgram', 'openai', 'gemini', 'custom'
